...
export function PageTitle({ title, subtitle, right }) {
  return (
    <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-center">
      <div>
        <h1 className="bg-[linear-gradient(135deg,#0f172a,#4338ca_55%,#06b6d4)] bg-clip-text text-3xl font-extrabold tracking-tight text-transparent md:text-[30px]">
          {title}
        </h1>
        {subtitle ? (
...
export function Panel({ children, className = "" }) {
  return (
    <section
      className={`rounded-[24px] border border-white/70 bg-white/78 p-5 shadow-[0_18px_50px_rgba(148,163,184,0.18)] backdrop-blur ${className}`}
    >
      {children}
    </section>
  );
...
export function SmallButton({
  ...
  return (
    <Comp
      className={`inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${className}`}
      {...props}
    >
      {children}
    </Comp>
...
export function PrimaryButton({
  ...
  return (
    <Comp
      className={`inline-flex items-center justify-center rounded-xl bg-[linear-gradient(135deg,#4f46e5,#0ea5e9)] px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-indigo-200 transition hover:-translate-y-0.5 hover:opacity-95 ${className}`}
      {...props}
    >
      {children}
    </Comp>
...
export function RoleHome() {
  return (
    <div className="grid gap-6 md:grid-cols-2">
      <Link
        to="/team3/student"
        className="rounded-[28px] border border-white/70 bg-[linear-gradient(135deg,#ffffff,#e0f2fe)] p-8 shadow-xl shadow-sky-100 transition hover:-translate-y-1"
      >
        <div className="mb-4 text-4xl">🎓</div>
        <h2 className="text-2xl font-bold text-slate-800">Оюутны UI</h2>
        <p className="mt-3 text-slate-600">
          Нүүр, дүнгийн мэдээлэл, ирц, хуанли, чөлөө болон актын дэлгэцүүд.
        </p>
      </Link>

      <Link
        to="/team3/teacher"
        className="rounded-[28px] border border-white/70 bg-[linear-gradient(135deg,#ffffff,#ede9fe)] p-8 shadow-xl shadow-violet-100 transition hover:-translate-y-1"
      >
        <div className="mb-4 text-4xl">🧑‍🏫</div>
        <h2 className="text-2xl font-bold text-slate-800">Багшийн UI</h2>
        <p className="mt-3 text-slate-600">
          Журнал, ирц бүртгэл, оюутны жагсаалт, хүсэлтийн удирдлагын дэлгэцүүд.
        </p>
      </Link>
    </div>
  );
...
export function EmptySettings() {
  return (
    <Panel className="flex min-h-[320px] items-center justify-center bg-[linear-gradient(135deg,#ffffff,#eff6ff)] text-xl font-semibold text-slate-500">
      Тохиргооны хэсэг
    </Panel>
  );
}
  ...
  const items = [
    {
      ...
      tone: "from-sky-100 to-cyan-100 text-sky-900",
    },
    {
      to: `${base}/breakdown`,
      ...
      {tabs.map(([key, label]) => (
        <Link
          ...
          className={`rounded-2xl px-6 py-3 text-sm font-semibold transition ${
            active === key
              ? "bg-[linear-gradient(135deg,#4338ca,#06b6d4)] text-white shadow-lg"
